...
description: >
  ...
  accent, the three-step gray text ladder, Mulish (display/numerals/chrome) + Mulish (body),
...
colors:
  ...
  text-muted: "#6B6B6B"
  text-light: "#9A9A9A"
  accent-light: "rgba(102, 56, 217, 0.08)"
  accent-medium: "rgba(102, 56, 217, 0.15)"
  border: "rgba(102, 56, 217, 0.2)"
  card-bg: "rgba(102, 56, 217, 0.04)"
  positive: "#059669"
  negative: "#dc2626"
...
typography:
  # — reading ramp (Mulish body + Mulish chrome) —
  body:    { fontFamily: "Mulish", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Mulish", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Mulish", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Mulish", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Mulish, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Mulish", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Mulish", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Mulish", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Mulish", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Mulish", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Mulish", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Mulish", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
- **Fonts** — already set to **Mulish** (display) / **Mulish** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{200, 300, 400, 500, 600, 700, 800, 900}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is two faces in fixed roles: **Mulish** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Mulish** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Mulish** (display/numerals/chrome) + **Mulish** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Mulish body 0.85cqw muted; Mulish eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Mulish `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Mulish 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Mulish `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Mulish label + muted desc + optional
...
blockquote, cite. **Focal** a Mulish `blockquote` near-black under a 15%-opacity cobalt
...
Mulish body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Mulish `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Mulish 400 muted, line 1.6.
...
- **Type** — Mulish headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Mulish body muted line 1.6; ≥1.4cqw floor.
...
- **Mulish + Mulish via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
...
## Font loading (corrected — brand body font is Mulish, not an icon font)

`build-frame.mjs` mis-picked **Mulish** (a WordPress/Elementor icon glyph font, `isIcon: true` in
`fonts-manifest.json`) as display+body. The real brand typeface on syllaby.io is **Mulish**
(200–900), confirmed live at `fonts.gstatic.com/s/mulish/...`. Load it directly from that
permanent Google-hosted CDN path (same font, same bytes the live site serves — not a generative
service, no key) at the weights this frame system needs. Paste this into every frame's
`<head>`/`<template>`:
