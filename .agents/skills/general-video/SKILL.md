...
**Multi-scene builds (roughly 4+ scenes or any expected multi-round fix loop): delegate this checklist, don't run it inline.** Read `../hyperframes-core/references/subagent-dispatch.md` and DISPATCH a child with this checklist + `PROJECT_DIR` as its task: it runs lint/validate/inspect, fixes findings itself (cheapest safe edit per finding, bounded to ~3 retries each, re-running only the failed check), and returns one summary line (pass/fail + what it fixed). You read back only that line — never the raw `--json` output or fix-loop transcript. This is what keeps a long multi-scene fix cycle from ballooning the main conversation's cache-read tokens (see `CLAUDE.md`'s "Session & token budget" section); for a small build (a title card, a one-off edit) running the checklist yourself inline is fine and simpler.
